  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals,
    ...
    if (jacobians) {
      ...
      if (jacobians[0]) {
        ...
        jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Utility::Qleft(Qj.inverse() * 
          Qi) * Utility::Qright(corrected_delta_q)).bottomRightCorner<3, 3>();
#endif
      ...
      if (jacobians[1]) {
        ...
        // lio中pre_integration->delta_q用的是corrected delta q
