...
void Estimator::setParameter() {
  ...
  // 配置文件中td的解释：initial value of time offset. unit: s. 
...
// xjw: 在 estimator_node 中的 process 函数中被调用，添加imu数据到 estimator
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration,
                           const Vector3d &angular_velocity) {
  if (!first_imu)  // 未获取第一帧IMU数据时，初始化前一帧imu数据
  {
    first_imu = true;
  ...
  if (!pre_integrations
          [frame_count])  // xjw:
                          // 初始化该帧的imu数据，应该只在first_imu时会发生吧
  {
  ...
  {
    // 将imu数据保存到预积分对象，并propagate 中值积分到当前时刻
    ...
    // 当成功完成了初始化后，下面这一部分积分才有用，它可以通过IMU积分得到滑动窗口中最新帧在世界坐标系中的位姿
    int j = frame_count;
...
void Estimator::processImage(
  ...
  // 只有在滑动整个塞满后，或者初始化完成之后，才需要滑动窗口，此时才需要做关键帧判别，根据第2最新关键帧是否未关键帧选择相应的边缘化策略
  ...
  all_image_frame.insert(make_pair(
      header.stamp.toSec(),
      imageframe));  // 每读取一帧图像特征点数据，都会存入all_image_frame
  tmp_pre_integration =
      new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count]};
  ...
  if (solver_flag == INITIAL)  // 需要初始化
  {
    if (frame_count == WINDOW_SIZE)  // 滑动窗口中塞满了才进行初始化
    {
      bool result = false;
      if (ESTIMATE_EXTRINSIC != 2 &&
          (header.stamp.toSec() - initial_timestamp) >
              0.1)  // 在上一次初始化失败后至少0.1秒才进行下一次初始化
      {
 ...
 *   选择跟当前帧中有足够多数量的特征点（30个）被跟踪，且由足够视差（2o
 * pixels）的某一帧，利用五点法恢复相对旋转和平移量。
...
bool Estimator::initialStructure() {
  ...
  for (int i = 0; frame_it != all_image_frame.end(); frame_it++) {
    ...
    if ((frame_it->first) ==
        Headers[i].stamp.toSec())  // all_image_frame与滑动窗口中对应的帧
    {
      frame_it->second.is_key_frame = true;  // 滑动窗口中所有帧都是关键帧
      frame_it->second.R =
          Q[i].toRotationMatrix() *
          RIC[0].transpose();  // 根据各帧相机坐标系的姿态和外参，得到用各帧IMU坐标系的姿态（对应VINS
                               // Mono论文(2018年的期刊版论文)中的公式（6））。
    ...
    frame_it->second.is_key_frame =
        false;  // 初始化时位于滑动窗口外的帧是非关键帧
    ...
    for (auto &id_pts : frame_it->second.points)  // 对于该帧中的特征点
    {
      ...
      for (
          auto &i_p :
          id_pts
              .second)  // 由于可能有多个相机，所以需要遍历。i_p对应着一个相机所拍图像帧的特征点信息
      {
    ...
    if (pts_3_vector.size() <
        6)  // 如果匹配到的3D点数量少于6个，则认为初始化失败
    {
    ...
    frame_it->second.R =
        R_pnp *
        RIC[0]
            .transpose();  // 根据各帧相机坐标系的姿态和外参，得到用各帧IMU坐标系的姿态。
...
bool Estimator::visualInitialAlign() {
  ...
  Matrix3d R0 = Utility::g2R(
      g);  // 当前参考坐标系与世界坐标系（依靠g构建的坐标系）的旋转矩阵，暂时每搞清楚从谁转到谁？？？
...
void Estimator::optimization() {
  ...
  for (int i = 0; i < WINDOW_SIZE + 1; i++) {
    ceres::LocalParameterization *local_parameterization =
        new PoseLocalParameterization();  //位姿约束在dof6
    problem.AddParameterBlock(para_Pose[i], SIZE_POSE,
                              local_parameterization);  // 位姿 - 7
    problem.AddParameterBlock(para_SpeedBias[i],
                              SIZE_SPEEDBIAS);  // 速度 bias_g bias_a - 9
  }
  //添加要优化的变量：相机到IMU的外参
  for (int i = 0; i < NUM_OF_CAM; i++) {
    ceres::LocalParameterization *local_parameterization =
  ...
  //添加要优化的变量：时间偏差
  if (ESTIMATE_TD)  // 相机与imu的时间offset
  {
  ...
  //将要优化的变量转为数组形式
  vector2double();  // 初始化para_Pose等优化参数

  //4.添加残差块 =================================================
  //4.1 添加边缘化的先验残差信息，第一次进行优化的时候last_marginalization_info还为空值
  ...
  for (int i = 0; i < WINDOW_SIZE; i++)  // 添加imu残差
  {
    ...
    IMUFactor *imu_factor =
        new IMUFactor(pre_integrations[j]);  //初始化残差实例的imu数据
    problem.AddResidualBlock(imu_factor, NULL,  //添加残差模型，和参数
                             para_Pose[i], para_SpeedBias[i], para_Pose[j],
                             para_SpeedBias[j]);
  }
  //4.3 遍历特征构建视觉重投影残差
  int f_m_cnt = 0;
  int feature_index = -1;
  for (auto &it_per_id : f_manager.feature)  // 特征点
  {
    it_per_id.used_num = it_per_id.feature_per_frame.size();
    if (!(it_per_id.used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 2))
      continue;
    ...
    for (auto &it_per_frame : it_per_id.feature_per_frame) 
    {
      ...
      Vector3d pts_j = it_per_frame.point;
      if (ESTIMATE_TD)  // true：优化camera数据与IMU数据的时间偏移
      {
  ...
  double2vector();  // 更新Vector3d类的状态量，基于para_Pose等优化参数数组
  ...
  if (marginalization_flag == MARGIN_OLD) {
    ...
    vector2double();  // 将Ps，Vs，Rs，Bas等Vector中的值，覆盖到para_Pose等优化参数数组中
  ...
  else 
  {
    ...
    if (last_marginalization_info &&
      ...
      //计算每次IMU和视觉观测(cost_function)对应的
      //参数块(parameter_blocks),雅可比矩阵(jacobians),残差值(residuals)
      marginalization_info->preMarginalize();
...
void Estimator::slideWindow() {
  TicToc t_margin;
  if (marginalization_flag == MARGIN_OLD)  // 移除最旧的关键帧
  {
    double t_0 = Headers[0].stamp.toSec();
    back_R0 = Rs[0];
    back_P0 = Ps[0];
    if (frame_count == WINDOW_SIZE)  // 窗口满时，移除最老的keyframe
    {
      for (int i = 0; i < WINDOW_SIZE; i++)  // 使用冒泡法，将第0帧移到最后
      {
        Rs[i].swap(Rs[i + 1]);
      ...
      // 此时数据排序[3 4 5 6 7]变为[4 5 6 7 3]，将最后一个数据用最新的数据覆盖，
      // 变为[4 5 6 7 7]，相当于删掉了最旧的一帧
      ...
      if (true || solver_flag == INITIAL)  // 移除对应img
      {
      ...
      slideWindowOld();  // 清除旧帧对应的特征点
    }
  } // end if (marginalization_flag == MARGIN_OLD)
  else //边缘化掉滑动窗口中次新的帧
  {
    if (frame_count == WINDOW_SIZE) {
      for (unsigned int i = 0; i < dt_buf[frame_count].size(); i++)  
      // 把最后一帧的imu数据append到倒数第二帧里
      {
